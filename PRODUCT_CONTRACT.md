...
Deliver an open-source native Windows and macOS application that lets a user
...
| `LCP-05` | A normal user can install, launch, diagnose, switch, restore, and uninstall the Beta on each supported platform. | Test packaged artifacts on real Windows and macOS hosts, including one custom Provider preflight/switch/restore flow and documented unsigned/signing limitations. |
...
`LCP-01` through `LCP-06` are all core. A fixed OpenAI/Qilin/VectorEngine
selector, a catalog data layer without an end-to-end UI flow, passing tests, or
